...
function replaceText(word, replacement, text) {
  ...
  const textChanged = text.replace(regexp, replacement)
...
function checkWord(word, text) {
  ...
  const textChecked = regexp.test(text)
...
function extractTextInParentheses(str) {
  ...
  const muches = [...str.matchAll(regexp)]
  // Створення масиву зі знайденими текстами.
  const arr = []
  for (const much of muches) {
    arr.push(much[1])
  }
...
function countEmails(str) {
  ...
  const muches = [...str.match(regexp)]
  // Підрахунок кількості email-адрес.
  const result = muches.length
...
function checkRegexFlags(regex) {
  ...
  const gmFlags = flags.includes('g') && flags.includes('m')
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const result = str.replace(regexp, newWord)
